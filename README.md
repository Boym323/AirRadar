...
The persistent official cache cannot refresh or bootstrap a new primary DDB
snapshot while upstreams are unavailable. Without the separately enabled
SoftRF whitelist, a new device therefore remains unresolved and hidden until
a valid targeted DDB response has been received.

As an explicitly enabled emergency fallback, `OGN_SOFTRF_DDB_PATH` may point
to the local SQLite `ogn.db` generated by the SoftRF project. AirRadar opens
it read-only, validates the `devices` schema and snapshot age, and indexes
