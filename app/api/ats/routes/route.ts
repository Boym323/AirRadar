...
export async function GET(): Promise<Response> {
  const documents = [loadCzAtsRoutes(), loadSkAtsRoutes(), loadAtAtsRoutes()].filter((value): value is NonNullable<typeof value> => value !== null);
  ...
  const source = documents.length === 1 ? documents[0].source : { ...documents[0].source, name: documents.map((document) => document.source.name).join(" + "), reference: documents.map((document) => document.source.reference).join(" | ") };
